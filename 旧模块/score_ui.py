    ...
    with sub_tabs[0]:
        ...
        if st.button("运行 Tracking", use_container_width=True):
            with st.expander("可选：自选指标打勾（TDX 表达式，逗号分隔；仅用于打勾，不影响样本）", expanded=False):
                exprs = st.text_input("表达式", value="", key="track_exprs")
                tf = st.selectbox("时间框架", ["D","W","M"], index=0, key="track_tf")
                winN = st.number_input("窗口长度", min_value=10, max_value=240, value=60, step=5, key="track_win")
                scope = st.selectbox("作用域", ["ANY","LAST"], index=1, key="track_scope")
            ...
            if st.button("生成跟踪表（含前日排行/名单/指标勾选/后续涨幅）", use_container_width=True):
                try:
                    ...
                    # 3) 合并“单子”（白/黑/特别关注）
                    try:
                        # 直接使用 scoring_core 提供的名单读取（注意这些函数在 scoring_core 内部）
                    ...
                    if exprs.strip():
                        ...
                        for i, e in enumerate(expr_list, start=1):
                            try:
                                # 以 tdx_screen 计算当日命中集合
                                hit_df = se.tdx_screen(ref_date=str(refT), when=e, timeframe=str(tf), window=int(winN),
                                                    scope=str(scope), return_df=True)
                                hits = set(hit_df["ts_code"].astype(str)) if ("ts_code" in hit_df.columns) else set()
                    ...
                    st.caption("提示：ret_fwd_N = 未来 N 日涨幅（基于前复权收盘），其计算已在 Tracking 内完成。")
                except Exception as e:
                    st.error(f"生成失败：{e}")
